...
vec3 calculeazaDeformarea(float hp, vec3 pozitie) {
    return hp * vec3(sin(pozitie.x), sin(pozitie.y), sin(pozitie.z));
}

void main()
{
    vec3 pozitieDeformata = v_position + calculeazaDeformarea(hp, v_position);

    // TODO(student): Compute world space vectors
    vec3 world_pos = (Model * vec4(pozitieDeformata, 1.0)).xyz;
